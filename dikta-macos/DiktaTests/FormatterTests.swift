...
// -- EmbeddingParagraphSplitter stub (test-local, always returns no breaks) --
// Used only so StructuredTextFormatter's embeddingSplitter property type resolves.
struct EmbeddingParagraphSplitter {
    func breakIndices(for sentences: [String]) -> [Int] { return [] }
}
